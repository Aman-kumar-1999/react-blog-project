...
export default function CBEXIII() {
  ...
  const createProductsInBulk = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/excel/cbexiii`, formData, {
        headers: {
      ...
      if (response.status == 200) {
        ...
        link.download = 'CBE-XIII-Generated.xml';
  ...
  return (
    <>
      ...
      <label className="label" aria-hidden="true">Select Your CBE-XIII Excel</label>

      <div style={{ float: 'right' }} className=''>
        <a href='Sample-CBE-XIII.xlsx' >
